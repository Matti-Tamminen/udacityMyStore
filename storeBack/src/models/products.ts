...
export default class Product {
	async createOneProduct(name: ProductType['name'], description: ProductType['description'], price: ProductType['price'], url: ProductType['url']): Promise<ProductType> {
		const connection = await client.connect()
		const sql = 'INSERT INTO products (name, description, price, url, quantity) VALUES ($1, $2, $3, $4, 1) RETURNING *;'
		const res = await client.query(sql, [name, description, price, url])
		connection.release()
